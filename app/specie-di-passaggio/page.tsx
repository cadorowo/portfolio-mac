    ...
    <main className="portfolio-stage vinyl-page-stage">
      <section className="vinyl-table" aria-label="Selezione vinili">
        ...
        <div className="vinyl-deck">
          <p className="vinyl-count" aria-live="polite">{String(activeIndex + 1).padStart(2, '0')} / {String(vinyls.length).padStart(2, '0')}</p>
          <div className="cover-stack" aria-label={`Vinile: ${activeVinyl.title}`}>
            {leavingIndex !== null && <img className="vinyl-cover vinyl-cover-leaving" src={vinyls[leavingIndex].src} alt="" aria-hidden="true" />}
